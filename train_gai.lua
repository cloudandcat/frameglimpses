...
cmd:option('-train_data_file','train_data_file.h5')

cmd:option('-val_data_file','val_data_file.h5')

cmd:option('-val_vids_file','val_vids_file.txt')
...
cmd:option('-num_classes',2,'number of classes in the data files (used to select pos/neg examples for each batch)')
cmd:option('-pos_class',2,'class to train a classifier for')
cmd:option('-seq_len',30,'number of frames in each video chunk')
cmd:option('-batch_size_pos',20,'number of examples from pos class in each batch')
cmd:option('-batch_size_neg',18,'number of examples from each neg class in each batch')
...
function eval_val_split(epoch, randomize_ap)
    ...
    for test_iter = 1,n do
        ...
        for b=1,num_batch_examples do
            ...
            local vidStartFrame = meta[b]['seq'][1]
            for i=1,#gts do
                local gt_start = gts[i][1]
                local gt_end =  gts[i][2]
            ...
            for t=1,opt.num_glimpses do
                ...
                local det_t = torch.round(torch.mul(predictions[t][2][b], 9))
                local conf_t = predictions[t][3][b][1]

                if use_det_t == 1 then
                    local det_start = det_t[1]
                    local det_end = det_t[2]table.insert(all_dets[vidName], {det_start, det_end})
    ...
    for i=1,numOrderedVids do
        local vidName = orderedVidNames[i]
        gts = all_gts[orderedVidNames[i]]
        dets = all_dets[orderedVidNames[i]]
        confs = all_confs[orderedVidNames[i]]

        for di = 1,# all_dets[orderedVidNames[i]] do
            local det_start = all_dets[orderedVidNames[i]][di][1]
            local det_end = all_dets[orderedVidNames[i]][di][2]
            conf = all_confs[orderedVidNames[i]][di]
            local mappedClassIdx = opt.pos_class
            if string.len(opt.class_mapping_file) > 0 then
              mappedClassIdx = classMapping[opt.pos_class]
            end
            dets_file:write(string.format('%s\t%.1f\t%.1f\t%d\t%.2f\n', orderedVidNames[i] ,all_dets[orderedVidNames[i]][di][1]/5 , all_dets[orderedVidNames[i]][di][2]/5, mappedClassIdx, all_confs[orderedVidNames[i]][di]))
        end
...
function compute_ap(tpconf, fpconf, total_num_gt, randomize_ap)
    ...
    for i=1,#sorted_conf_vals do
        ...
        if num_fp_counts > 0 then
         ...
         if num_fn_counts > 0 then
           ---still vaguelly
            sorted_conf[{3, {conf_iter+num_tp_counts, conf_iter+total_conf_counts-1}}]:fill(3)
        end
    ...
    -----still vaguelly
    fn = torch.cumsum(sorted_conf[2]:eq(2):double())
